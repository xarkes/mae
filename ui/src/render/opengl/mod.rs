...
mod android {
    ...
    pub fn ogl_create_context(win: &Window) -> GLContextHandle {
        ...
        let lib =
            unsafe { libloading::Library::new("libEGL.so").expect("unable to find libEGL.so") };
...
impl GLContext {
    pub fn new(win: &Window) -> Self {
        ...
        unsafe {
            ...
            let renderer = gl::GetString(gl::RENDERER) as os_impl::GLStringPtr;
            if vendor != std::ptr::null_mut() && version != std::ptr::null_mut() {
                ...
                let rendererstr = std::ffi::CStr::from_ptr(renderer).to_str().expect("<err>");
                log::info!(
...
extern "system" fn gl_debug_func(
    ...
    log::debug!("Opengl error: {} {}", src, decoded)
}
